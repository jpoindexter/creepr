...
const CATEGORY_CONFIG: Array<{ key: string; title: string; color: string }> = [
  ...
  { key: "divide", title: "Divide", color: "muted" },
...
const colorClasses: Record<string, { bg: string; border: string; badge: string }> = {
  ...
  muted: { bg: "bg-muted", border: "border-border", badge: "bg-muted text-foreground" },
};
...
function InconsistencyCard({ inconsistency }: { inconsistency: Inconsistency }) {
  ...
  return (
    <div className={`rounded-none border ${config.border} ${config.bg}`}>
      <button
      ...
      >
        <div className="flex items-start gap-4">
          ...
          <div className="text-left">
            <div className="text-foreground font-medium">{inconsistency.description}</div>
            <div className="text-muted-foreground mt-1 text-sm">{inconsistency.recommendation}</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className={`rounded-none px-2 py-0.5 text-xs font-medium ${config.badge}`}>
      ...
      {expanded && (
        <div className="border-border border-t p-4 text-sm">
          <div className="mb-4 flex items-center gap-2">
            <span className="font-medium text-green-700">Dominant:</span>
            <code className="rounded-none bg-green-100 px-2 py-0.5 text-green-800">
          ...
          <div className="space-y-2">
            ...
            {inconsistency.outliers.map((outlier, i) => (
              <div key={i} className="bg-card ml-4 rounded-none p-2">
                <div className="flex items-center gap-2">
                  <code className="rounded-none bg-red-50 px-2 py-0.5 text-red-700">
                    {outlier.pattern}
                  </code>
                  <span className="text-muted-foreground">
                ...
                <div className="mt-1 space-y-0.5">
                  {outlier.files.map((f, j) => (
                    <div key={j} className="text-muted-foreground text-xs">
                      {f.file}:{f.line}
                    </div>
                  ))}
                </div>
...
export function AuditResultsView({ report }: AuditResultsViewProps) {
  ...
  const getCategorySummary = () => {
    ...
    for (const config of CATEGORY_CONFIG) {
      ...
      const allCodes = Object.values(category).flatMap((c: AuditCategory) =>
        c.violations.map((v) => v.code)
      );
      const uniqueCount = new Set(allCodes).size;
      const totalCount = Object.values(category).reduce(
        (sum: number, c: AuditCategory) => sum + c.count,
        0
      );
    ...
    <div className="bg-muted flex h-full flex-col">
      ...
      <div className="border-border bg-card border-b px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-foreground text-xl font-semibold">Design System Audit Results</h2>
        ...
        <div className="mt-4 flex items-center gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="border-border bg-card focus:border-primary focus:ring-primary w-full rounded-none border py-2 pr-4 pl-10 text-sm focus:ring-1 focus:outline-none"
            />
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {report.inconsistencies && report.inconsistencies.totalInconsistencies > 0 ? (
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  ...
                  {report.inconsistencies.critical > 0 && (
                    <span className="ml-2 rounded-none bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800">
                      {report.inconsistencies.critical} critical
                    </span>
                  )}
                  {report.inconsistencies.warning > 0 && (
                    <span className="ml-2 rounded-none bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">
          ...
          <div className="mb-8 rounded-none border border-green-200 bg-green-50 p-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-green-700">
                  Your design system patterns are consistent. The patterns below show your current
                  usage.
                </div>
              </div>
            </div>
          ...
          <>
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {categorySummary.map(({ key, title, uniqueCount, color }) => (
                <button
                  ...
                  className={`rounded-none border p-4 text-left transition-all hover:opacity-80 ${colorClasses[color]?.bg || "bg-muted"} ${colorClasses[color]?.border || "border-border"}`}
                >
                  <div className="text-foreground text-2xl font-bold">{uniqueCount}</div>
            ...
            <div className="space-y-4">
              {CATEGORY_CONFIG.map(({ key, title, color }) => {
                ...
                const allCodes = Object.values(filteredCategories).flatMap((c: AuditCategory) =>
                  c.violations.map((v) => v.code)
                );
                const uniqueCount = new Set(allCodes).size;
                ...
                return (
                  <div
                    key={key}
                    className={`rounded-none border ${colorClasses[color]?.border || "border-border"} ${colorClasses[color]?.bg || "bg-card"}`}
                  >
                    <button
                      onClick={() => toggleCategory(key)}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`rounded-none px-4 py-1 text-sm font-medium ${colorClasses[color]?.badge || "bg-muted text-foreground"}`}
                        >
                          {uniqueCount} unique
                        </span>
                      ...
                      <div className="border-border border-t p-4">
                        {Object.entries(filteredCategories)
                          ...
                          .map(([subKey, cat]) => {
                            ...
                            const sortedCodes = Object.entries(codeGroups).sort(
                            ...
                            return (
                              <div key={subKey} className="mb-4 last:mb-0">
                                <div className="mb-2 flex items-center justify-between">
                                  <h4 className="text-foreground font-medium">{cat.description}</h4>
                                  <span className="text-muted-foreground text-sm">
                                    {Object.keys(codeGroups).length} unique patterns
                                  </span>
                                </div>
                                ...
                                {showFilesOnly ? (
                                  ...
                                  <div className="space-y-1">
                                    {sortedCodes.slice(0, 20).map(([code, violations]) => (
                                      <div key={code} className="bg-card rounded-none p-2">
                                        <div className="flex items-start justify-between">
                                          <code className="bg-muted rounded-none px-2 py-1 text-sm">
                                            {code}
                                          </code>
                                    ...
                                    {sortedCodes.length > 20 && (
                                      <div className="text-muted-foreground/60 text-sm">
                                        +{sortedCodes.length - 20} more patterns
                                      </div>
                                    )}
                                  </div>
                                ) : (
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {sortedCodes.slice(0, 30).map(([code, violations]) => (
                                      <span
                                        key={code}
                                        className="bg-card inline-flex items-center gap-1 rounded-none px-4 py-1 text-sm"
                                    ...
                                    {sortedCodes.length > 30 && (
                                      <span className="text-muted-foreground inline-flex items-center px-4 py-1 text-sm">
