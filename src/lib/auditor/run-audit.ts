 ...
 * Main function to run the design system audit
 */

import type { AuditCategory, DesignSystemAuditReport } from "./types";
import { AUDIT_PATTERNS } from "./patterns";
import { findPatterns, countUniquePatterns, detectAllInconsistencies } from "./helpers";
...
export function runDesignSystemAudit(
  ...
  let totalPatterns = 0;
  const allViolations: Array<{ code: string }> = [];
  ...
  report.patternSummary = countUniquePatterns(
    allViolations as Array<{ file: string; line: number; code: string }>
  );
