...
export async function runAIAudit(
  ...
  console.log(
    `[AI Auditor] Using ${isLargeModel ? "reduced" : "standard"} context: ${MAX_CONTENT_LENGTH / 1000}KB`
  );
  ...
  for (const file of prioritizedFiles) {
    ...
    if (/\/(lib|utils|hooks|services|api)\//i.test(file.path) && !/\.css$/i.test(file.path))
      continue;
  ...
  console.log(
    `[AI Auditor] Will analyze ${filesToAnalyze.length} files (${(totalLength / 1024).toFixed(1)}KB)`
  );
...
${filesContent}`;
  ...
  console.log(
    `[AI Auditor] Estimated tokens: ${estimatedTokens}, requesting context: ${contextSize}`
  );
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[AI Auditor] Parsed successfully, found ${parsed.inconsistencies?.length || 0} inconsistencies`
      );
    } catch (parseError) {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      ...
      throw new Error(
        "AI analysis timed out. The model took too long to respond. Try a smaller model or fewer files."
      );
    }
    console.error("[AI Auditor] ERROR:", error);
    console.error(
      "[AI Auditor] Error type:",
